...
fn launch_ai_sidecar(_app: AppHandle, port: u16) -> Result<String, String> {
    ...
    let venv_python = exe_dir.join("ai_sidecar").join("venv").join("Scripts").join("python.exe");
    ...
    let (py, py_script) = if venv_python.exists() && sidecar_py.exists() {
        ...
        let v = fb.parent().unwrap().join("venv").join("Scripts").join("python.exe");
        (v, fb.clone())
    } else {
    ...
    match cmd.spawn() {
        Ok(_) => Ok(format!("AI sidecar launched on port {}", port)),
        Err(e) => Err(format!(
